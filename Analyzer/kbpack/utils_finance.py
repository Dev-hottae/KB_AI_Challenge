...
def convert_texts_to_features(
        finance_texts: List[InputExample],
        max_seq_len: int,
        tokenizer: PreTrainedTokenizer,
    ...
    features = []
    for (ex_index, finance_text) in enumerate(finance_texts):
        tokens = tokenizer.tokenize(finance_text.text_a)
